...
- [ ] T-39 — **os comentários de `Q-09` param de dizer que a questão está aberta, e o gatilho de `D-107` vira portão** (`D-107`/`QA-07`) · **Módulo:** M7 (+ o comentário de M5) · **Por que agora:** é a régua de `D-105`, de ontem — comentário que declara lacuna vira dívida no minuto em que a lacuna fecha, e nada no portão acusa. Hoje **6 arquivos** de `src/` citam `Q-09`, e `derivacao.ts` diz em quatro linhas que resolver de vez é `pending()` e que "enquanto `Q-09` estiver aberta, é aqui que a lacuna mora": verdadeiro até 2026-09-01, falso depois [Fonte: `grep -rn "Q-09" src/`; src/ui/derivacao.ts:8-26] · **Fazer 1:** trocar essas citações pelo que `D-107` decidiu — a derivação **é** a resposta —, apontando [[m7_tela_notas]] em vez de repetir o raciocínio; comentário e nada de lógica · **Fazer 2 — o gatilho vira grep no CI**, junto das checagens de camada do `pages.yml`: `grep -rlE "from '[^']*session" src/ --include=*.ts | grep -v "^src/ui/\|^src/tests/" | wc -l` tem de dar **0**, e reprovar acima disso dizendo que `Q-09` reabriu como `D-NN` novo [Fonte: .github/workflows/pages.yml:58-78] · **Portão — três, e a do meio prova as outras:** (1) `grep -rn "Q-09" src/` não devolve linha dizendo que a questão está **aberta** ou que é **do dono**; o que sobrar cita `D-107` · (2) **liga/desliga:** com um `import` de `src/session/` plantado fora de `src/ui/` e `src/tests/`, o passo novo **reprova**; removido, passa — grep nunca visto reprovando é decoração · (3) suíte **679/679** e `tsc --noEmit && build` verdes: nenhuma lógica entrou · **NÃO faz:** não acrescenta `pending()` (rejeitado por `D-107`) nem mexe no teste de `derivacao.ts`, que já prende o comportamento
