...
1. **O histograma da CPU atravessa as disputas do torneio, ou zera a cada uma?** `D-60` tirou a progressão **explícita** — mas pode existir uma implícita, e ninguém decidiu. `D-10` diz que o histórico de zonas vive "em memória, escopo da sessão", e um torneio são de 3 a 7 disputas do jogador: se M8 reusar o mesmo `Cpu`, a CPU chega à final tendo lido o jogador a partida inteira e fica mais difícil sozinha — exatamente o que `D-60` recusou. Se criar um `Cpu` por disputa, cada adversário começa cego. É contrato de M8 e vale mais que qualquer ajuste de nível.
