...
1. **A dificuldade sobe ao longo do torneio, ou é a que o jogador escolheu?** O portão de M8 em `D-13` dizia "nenhuma **progressão** de dificuldade passa dos 70%" — a palavra estava lá, mas a progressão nunca foi decidida. Hoje `TournamentConfig.level` é **um** valor, e um valor só significa "não sobe": grupos e final na mesma dificuldade. Se subir, o campo vira faixa e o teto de `D-10` passa a ser cobrado no topo dela, não no meio. É contrato de M8, não detalhe de `T-12`.
