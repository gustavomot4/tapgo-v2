...
| Servidores **STUN** padrão do `trystero@0.25.3` — `stun.l.google.com:19302`, `stun1.`, `stun2.` e `stun.cloudflare.com:3478` | cada navegador descobrir o próprio endereço público antes do WebRTC (ICE) | pública, de terceiros, sem conta e sem chave — é o `defaultIceServers` de `@trystero-p2p/core@0.25.3`, e M6 sobrescreve **só** `turnConfig`, então os 4 vão ao ar como padrão. **segunda exceção nominal** do portão de privacidade de M9 por `D-71` — descoberta de endereço é parte da conexão, não endpoint novo. A isenção vale para **esta** versão: subiu a dependência, reconfira a lista | Não | `node_modules/@trystero-p2p/core/dist/peer.mjs:302`, lido no `dist/` publicado (`assets/index-CvZbN4Im.js`) |
