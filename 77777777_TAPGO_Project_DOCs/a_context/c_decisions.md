...
| D-70 | 2026-08-19 | ADOTADO | `A-17`: **E-5 fecha** com o torneio aprovado no aparelho, e a perda de estado ao fechar o **Chrome do desktop** entra como ambiente, não como defeito | O mesmo build guardou o torneio no **Edge** (Chromium, mesmo `localStorage`) e no Chrome do celular, e fechar a **aba** do Chrome desktop também guardava — defeito de código reprovaria os três, então sobra a política de dados do perfil |
| D-71 | 2026-08-19 | ADOTADO | `Q-14`: os **4 STUN padrão** são a **segunda exceção nominal** do portão de privacidade de M9 — descoberta de endereço é parte da conexão de M6, não endpoint novo; a isenção nomeia `defaultIceServers` de `@trystero-p2p/core@0.25.3` | Sem STUN não há P2P: os 17/17 de `A-08` são `srflx`. Fixar `iceServers` (saída b) escolheria **os mesmos** servidores públicos à mão — muda quem digitou, não o que vai ao ar. A isenção cita arquivo e versão de propósito: o achado não foi "STUN é risco", foi **default de dependência não aparece em revisão**, e assim a próxima subida de versão reabre a conferência |
| D-72 | 2026-08-19 | ADOTADO | **E-6 entrega sem a tela de convite**, com a lacuna declarada: o online está **medido e não publicado**. O Objetivo do CONTEXT deixa de prometer "por link de convite"; a promessa volta quando `Q-11` tiver `D-NN` | O critério de aceite do online é a **medição**, e ela passou (`A-08`: 17/17, limite inferior 95% de 83,8% contra o corte de 70%, `D-47`). O que falta não é qualidade, é uma tela — e as duas saídas de `Q-11` mexem em porta congelada de M5 (`D-13`), logo exigem decisão sua e sessão de código. Segurar a entrega inteira por isso pararia CPU, 2P local e o torneio, que estão prontos e conferidos no aparelho. O que **não** era aceitável era entregar em silêncio: objetivo que promete o que o build no ar não faz é achado de entrega, não detalhe |
