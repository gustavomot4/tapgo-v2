...
| QA-08 | 2026-08-08 | CRÍTICO | `src/medicao.ts` (`D-33`) | O anfitrião descarta o `id` rotacionado e chama `hostRoom()`, que sorteia sala nova a cada toque: os dois aparelhos nunca se encontram e a medição de `A-08` dá 0% | Anfitrião tem de abrir a sala rotacionada, e a porta de M6 não aceita `roomId` em `hostRoom` (`D-13`) — é sessão de M6, ver [[m6_transporte_notas]] | _(aberto)_ |
