...
|---|---|---|
| `npx tsc --noEmit && npm run build` | dono | verdes na máquina do dono; é o que publica pelo Actions a cada `push`. **Com a ressalva já escrita:** `QA-04` segue aberto — o `include` do `tsconfig.json` não alcança o `vite.config.ts`, então este critério passa verde sobre um arquivo que ele não leu |
| Bundle inicial < 8 MB, lido da saída | agente + dono | **408.094 B = 5,10%** do teto, relido do `dist/` e não estimado. 278.646 B são os 32 SVGs de `T-19`; Phaser e Trystero ficam fora do inicial por `import()` (`D-27`). **Ressalva:** `QA-06` — a medição soma toda entrada `isEntry`, inclusive `medicao.html`, que o jogador nunca abre; o número real do jogador é menor, nunca maior |
| Disputa completa termina certa e roda 2x igual | agente | suíte **531/531**, com o determinismo rodando a linha do tempo duas vezes e comparando; o torneio inteiro conta 64 disputas em 6 sementes, e restaurar no meio chega à **mesma** linha do tempo (`D-57`) |
