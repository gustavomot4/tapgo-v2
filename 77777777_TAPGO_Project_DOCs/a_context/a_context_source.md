...
- **Próximo:** `T-24` (tempo por cobrança no `online`, `Q-15`) espera só o `D-NN` do dono — a saída (b) já é a dele, e `D-82` deu registro para ela · portão de `D-83` correndo: **1 de 10** (`QA-30`, 390; o script cobra também `Q-NN`, e `Q-15` mede 241) · a promessa que `D-72` tirou volta ao Objetivo quando couber
