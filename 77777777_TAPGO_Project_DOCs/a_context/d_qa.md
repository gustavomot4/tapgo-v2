...
| QA-25 | 2026-08-20 | ALTO | `src/session/index.ts:313` (M5, desde `T-13`) | `D-35` só é traduzido em `'failed'`: peer que sai chega como `'waiting'` (`net/index.ts:390`, que ainda REARMA os 20 s) e a disputa segue normal, sem "O outro jogador saiu". E `onPeerJoin` guarda só `'closed'`/`'failed'`: quem reabre o link dentro da janela é aceito com estado ZERADO contra um lado no meio da disputa | M5 tratar `'waiting'` **depois de já ter conectado** como abandono, ou M6 não rearmar — é porta entre M5/M6 e sai de outra sessão (regra 4) | _(aberto)_ |
