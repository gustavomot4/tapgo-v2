...
| QA-43 | 2026-08-29 | MÉDIO | `check.py:425-431` × `arquivar.py` | Uma régua em cada script para o mesmo `D-43`: o `check.py` conta `e_qa/` como citação viva, inclusive o `backlog_archive.md` "Somente leitura", e o `arquivar.py` não. Sete REJEITADAS só escapam por isso | `D-101`: o aviso só oferece o que o `arquivar.py` sem flag retiraria. Portão em [[qa43_uma_regua_so]] | _(aberto — falta implementar)_ |
