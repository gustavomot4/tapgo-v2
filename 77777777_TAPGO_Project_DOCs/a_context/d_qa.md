...
| QA-25 | 2026-08-20 | ALTO | `src/session/index.ts:313` (M5, desde `T-13`) | `D-35` só é traduzido em `'failed'`: peer que sai chega como `'waiting'` (`net/index.ts:390`, que ainda REARMA os 20 s) e a disputa segue normal **por 20 s** — a mensagem só chega quando o timer converte em `'failed'`. E `onPeerJoin` guarda só `'closed'`/`'failed'`: quem reabre o link dentro da janela é aceito com estado ZERADO contra um lado no meio da disputa | Trava permanente, **não** placar mentiroso: as guardas de `aoMove` seguram, e o que falta é o lado preso descobrir que quem voltou é sessão nova. Medições, saídas e a lacuna em [[qa25_reentrada_na_janela]] — é `D-NN` do dono (regra 6) | _(aberto)_ |
