...
| QA-40 | 2026-08-29 | BAIXO | `a_context_source.md`, a linha "Bloqueado/pendente" | As três ocupações de `D-97` estavam velhas (registro 15.6k contra 16.5k medidos) e nada as cobrava: a checagem do `check.py` casa `n/teto` em dígitos, e o `20k` à mão não casa | Números reescritos pela medida do disco; falta o portão — a linha escreve o teto em dígitos, ou a checagem entende o `k` | _(aberto)_ |
