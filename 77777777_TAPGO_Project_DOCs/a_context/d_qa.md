...
| QA-31 | 2026-08-20 | BAIXO | `src/ui/tela_cobranca.ts` (M7, desde `T-10`) | O "Sair da disputa" tem 144px de largura num bloco de 336: o `<button>` não estica porque o pai é um `<div class="empurra">`, e não um `.grupo` de flex como nas outras telas | Pôr `grupo` na classe do bloco. Medido igual ANTES e DEPOIS de `D-86`, nos dois viewports — não é do desktop (regra 4) | _(aberto)_ |
