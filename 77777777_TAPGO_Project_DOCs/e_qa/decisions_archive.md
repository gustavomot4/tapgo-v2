...
## Retiradas da tabela em 2026-08-29
...
| D-94 | 2026-08-28 | ADOTADO | Teto do BACKLOG 12.000 -> **20.000**, declarado em `.kit-config.json` | Arquivamento esgotado: 193.571 -> 15.365 (74 cards viram ponteiro) e os `P-NN` saem para [[pedidos_do_dono_260820]]. Sobra folga de ~4.600 |

## Corte de `D-74` no QA — os 10 achados FECHADOS saem da tabela viva (2026-08-29)

Mesma regra do corte de 2026-08-20: *"os FECHADOS saem, os abertos ficam"*. O QA estava em
**7.398/8.000** (92%) e o portão desta sessão pedia nenhum orçamento acima de 90%. Nenhum ID sumiu
e nada foi revertido: a íntegra é esta, o ponteiro é a linha "Retirados da tabela em 2026-08-29"
do cabeçalho do [[d_qa|QA]], e `check.py` resolve os dez por aqui (checagem 10 aceita ID definido
só nesta página). Ficam vivos os cinco abertos: `QA-07`, `QA-17`, `QA-27`, `QA-29` e `QA-34`.

| # | Data | Sev. | Onde | O que quebrava | Correção | Fechado em |
|---|---|---|---|---|---|---|
| QA-10 | 2026-08-08 | MÉDIO | `src/medicao.ts` (`D-33`) | Erro de configuração soma tentativa **e** falha: erro do operador entra na taxa que decide a revisão de `D-01`, enviesando para baixo | `D-96`/`T-38`: a base é recusada ANTES da 1ª tentativa (o canal nunca aberto sai do denominador) e as descartadas viram contador próprio | 2026-08-28 (`T-38`; 2 números + `A-40`, [[qa10_denominador_da_medicao]]) |
...
| QA-35 | 2026-08-21 | BAIXO | `b_process/c_backlog.md`, o card `A-30` | A caixa está `- [ ]` e o texto do card diz **`✔ FECHADA em 2026-08-21`** com o relato do dono junto: a marca contradiz o card, e quem varre por caixa vazia lê uma ação do dono que não existe mais | Marcar `- [x]`, sem tocar no texto | **FECHADO 2026-08-28** — caixa marcada, texto intacto |
